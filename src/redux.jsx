...
export const connect = (selector) => (Component) =>{  //柯里化
    return (props)=>{  //接受一个组件，返回一个组件
        ...
        //订阅store
        useEffect(()=>{
            const unsubscribe = store.subscribe(()=>{   //第一次执行时，会订阅store(就是传一个回调函数给store先存起来),当数据变化后，会先触发dispatch改变数据，然后再执行这个回调函数
            ...
            return unsubscribe   //useEffect有return的话，会在组件销毁之前执行；如果有多次渲染，就会在执行下一个effect之前，上一个effect被清除的时候执行return的函数
